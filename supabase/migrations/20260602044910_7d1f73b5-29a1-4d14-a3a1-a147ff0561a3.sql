...
-- Asegurar motivo "Caducidad" para mermas
INSERT INTO motivos_ajuste (nombre, tipo, activo)
SELECT 'Caducidad', 'merma', true
WHERE NOT EXISTS (SELECT 1 FROM motivos_ajuste WHERE nombre = 'Caducidad');

-- Reemplazar process_pos_sale: ignora lotes vencidos y acepta lote_id preferido por item
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    -- FEFO: lote preferido primero, luego por fecha de caducidad ASC, EXCLUYE vencidos
    FOR v_lote IN
