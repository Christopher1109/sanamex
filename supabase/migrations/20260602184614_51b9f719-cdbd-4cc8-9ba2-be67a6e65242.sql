-- 1) Add lista_precio_aplicada to ventas (text)
ALTER TABLE public.ventas
  ADD COLUMN IF NOT EXISTS lista_precio_aplicada text;

CREATE INDEX IF NOT EXISTS idx_ventas_lista_precio ON public.ventas(lista_precio_aplicada);

-- 2) Update process_pos_sale to capture price list from client type
...
-- 3) Backfill existing sales with LP based on client type (or LP1 default)
UPDATE public.ventas v
SET lista_precio_aplicada = CASE LOWER(COALESCE(c.tipo, 'publico'))
  WHEN 'publico' THEN 'LP1'
  WHEN 'mayoreo' THEN 'LP2'
  WHEN 'especial' THEN 'LP3'
  WHEN 'institucional' THEN 'LP4'
...
WHERE v.cliente_id = c.id AND v.lista_precio_aplicada IS NULL;

UPDATE public.ventas SET lista_precio_aplicada = 'LP1'
WHERE lista_precio_aplicada IS NULL;