-- Arregla "Error al cargar cotizador: window functions are not allowed in
-- window definitions". Bug preexistente (desde la migración del 24-jul-2026,
-- nadie lo había disparado en ejecución real hasta hoy): la CTE precios_score
-- llamaba MAX(precio_cmp) OVER (...) / MIN(precio_cmp) OVER (...) directamente
-- dentro del ORDER BY de otra función de ventana (row_number() OVER (...)).
-- Postgres no permite anidar una función de ventana dentro de la definición
-- de otra. Se separa en una CTE intermedia (precios_bounds) que calcula esos
-- MIN/MAX como columnas normales, y la siguiente CTE ya solo los referencia.
  ...
  precios_score AS (
    SELECT pb.*,
      row_number() OVER (
        ...
        ORDER BY (
          v_peso_precio * (CASE WHEN max_precio = min_precio THEN 0
            ELSE (precio_cmp - min_precio) / NULLIF(max_precio - min_precio, 0) END)
          + v_peso_entrega * (CASE WHEN max_dias = min_dias THEN 0
            ELSE (dias_entrega - min_dias)::numeric / NULLIF(max_dias - min_dias, 0) END)
        ) ASC, precio_cmp ASC
      ) AS rank
    FROM precios_bounds pb
  ),
  ultimo_costo AS (
