...
--    El gerente/almacenista la usa cuando el camión llega físicamente pero
--    todavía no se sienta a capturar lote/caducidad/factura. No mueve
--    inventario ni estado de la OC, es solo informativa.
...
-- 2) Quitar el "stand by": recibir_oc vuelve a exigir factura (sin ella no
--    hay forma de meter mercancía a inventario). Se elimina la variante que
--    permitía factura nula y el flujo de "ligar después" — ligar_factura_recepcion
--    se da de baja porque ya no tiene para qué existir.
...
-- 3) Bitácora de recepción — reemplaza el forms manual que hoy llenan los
--    gerentes. Se llena automáticamente al terminar de recibir (botón final),
--    y también admite alta manual para cuando no se hizo desde el sistema.
CREATE TABLE IF NOT EXISTS public.bitacora_recepcion (
  ...
  resumen_recibido jsonb, -- [{sku, nombre, cantidad}], snapshot de lo recibido
  reportado_por uuid,
...
-- Reporte automático: toma todo lo que ya se capturó de la OC (proveedor,
-- folio y monto de factura, folio y total de la orden, y el desglose de lo
-- recibido) y lo manda a la bitácora en un solo click.
...
-- 4) Promociones automáticas recurrentes ("Día Sanamex" y similares).
--    Configuración: clasificación o estatus + día de la semana + % de
--    descuento. La vigencia se calcula en vivo contra el día de hoy — no
--    hay que activar/desactivar nada a mano ni depender de un job.
CREATE TABLE IF NOT EXISTS public.promociones_precio (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  nombre text NOT NULL,
  dia_semana int NOT NULL CHECK (dia_semana BETWEEN 0 AND 6), -- 0=domingo … 6=sábado (EXTRACT(DOW))
...
-- Vista de conveniencia: promociones vigentes hoy, con el precio ya
-- calculado por producto que cumple el criterio. Pensada para que la
-- pantalla de ventas/cotizador la consulte directo sin reimplementar la
-- lógica de "qué día es hoy" en cada lugar donde se necesite el precio.
